...
var SocialIcon = React.createClass({
  ...
  render: function() {
    var outputIcon = Object.keys(this.state.networkImages).map(function(networkName){
      if (networkName == this.props.network) {
        return <div key={networkName}>{this.state.networkImages[networkName]}</div>
      }
    }.bind(this));
    return (
      <div>{outputIcon}</div>
