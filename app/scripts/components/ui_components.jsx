...
var UiComp = React.createClass({
  getInitialState: function() {
    return {
      images: {
        ...
        location: (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 47.825 47.825" className='ui__comp'>
        ...
        time: (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 612 612" className='ui__comp'>
  ...
  render: function() {
    var outputImage = Object.keys(this.state.images).map(function(imageName){
      if (imageName == this.props.image) {
        return <div key={imageName}>{this.state.images[imageName]}</div>
      }
    }.bind(this));
